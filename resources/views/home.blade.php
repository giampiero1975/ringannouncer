@php
    $homeSeoTitle = $homePage?->seo_title ?: 'RingAnnouncer Valerio';
    $homeSeoDescription = $homePage?->seo_description
        ?: \Illuminate\Support\Str::limit(trim(strip_tags((string) ($homePage?->content ?: 'RingAnnouncer Valerio - una voce oltre il ring.'))), 155);
@endphp
<!DOCTYPE html>
<html lang="it">
<head>
...
@php
    ...
    $images = [
        ...
        'calendarUnderlay' => $imageAsset('images/ringannouncer/calendar-underlay-user.png'),
        'videoBg' => $imageAsset('images/ringannouncer/video-section-bg.png'),
        'homeEditorialBg' => $imageAsset('images/ringannouncer/home-editorial-valerio-bg.png'),
    ];
    ...
    $dynamicEvents = $eventsSource->map(fn ($event, $index) => [
            'title' => $event->title,
    ...
    $fallbackGalleryImages = collect([
        ['url' => $imageAsset('images/ringannouncer/gallery-main-user.png'), 'title' => 'Momento dal ring RingAnnouncer 1', 'alt' => 'Momento dal ring RingAnnouncer 1'],
        ['url' => $imageAsset('images/ringannouncer/gallery-thumb-1-user-card.png'), 'title' => 'Momento dal ring RingAnnouncer 2', 'alt' => 'Momento dal ring RingAnnouncer 2'],
        ['url' => $imageAsset('images/ringannouncer/gallery-thumb-2-user-card.png'), 'title' => 'Momento dal ring RingAnnouncer 3', 'alt' => 'Momento dal ring RingAnnouncer 3'],
        ['url' => $imageAsset('images/ringannouncer/gallery-thumb-3-user-card.png'), 'title' => 'Momento dal ring RingAnnouncer 4', 'alt' => 'Momento dal ring RingAnnouncer 4'],
        ['url' => $imageAsset('images/ringannouncer/gallery-thumb-4-user-card.png'), 'title' => 'Momento dal ring RingAnnouncer 5', 'alt' => 'Momento dal ring RingAnnouncer 5'],
    ]);
    $galleryImages = $homeGalleryMedia->isNotEmpty()
        ? $homeGalleryMedia->map(fn ($media) => ['url' => $media->public_url, 'thumb' => $media->thumbnail_url, 'title' => $media->display_title, 'alt' => $media->alt_text ?: $media->display_title])
        : $fallbackGalleryImages;
    $navItems = collect([
        ['label' => 'Home', 'href' => '#home', 'active' => true],
        ['label' => 'Chi sono', 'href' => $pageLinks['chi-sono'] ?? '#bio'],
        ['label' => 'Eventi', 'href' => '#eventi'],
        ['label' => 'Gallery', 'href' => '#gallery'],
        ['label' => 'Video', 'href' => '#video', 'show' => $videos->isNotEmpty()],
        ['label' => 'Curiosità', 'href' => '#curiosita', 'show' => $articles->isNotEmpty()],
        ['label' => 'Contatti', 'href' => $pageLinks['contatti'] ?? '#contatti'],
    ])->filter(fn ($item) => $item['show'] ?? true);
    $footerNavItems = collect([
        ['label' => 'Home', 'href' => route('home'), 'active' => false],
        ['label' => 'Chi sono', 'href' => $pageLinks['chi-sono'] ?? '#bio', 'active' => false],
        ['label' => 'Eventi', 'href' => route('events.index'), 'active' => false],
        ['label' => 'Gallery', 'href' => route('galleries.index'), 'active' => false],
        ['label' => 'Video', 'href' => '#video', 'show' => $videos->isNotEmpty(), 'active' => false],
        ['label' => 'Curiosità', 'href' => route('articles.index'), 'show' => $articles->isNotEmpty(), 'active' => false],
        ['label' => 'Contatti', 'href' => $pageLinks['contatti'] ?? '#contatti', 'active' => false],
    ])->filter(fn ($item) => $item['show'] ?? true);
@endphp
...
<header id="home" class="hero">
    ...
    <div class="topbar"><div class="wrap"><a class="brand" href="#home"><img src="{{ $imageAsset('images/ringannouncer/logo-valerio-header.png') }}" alt="RingAnnouncer Valerio Lamanna" width="1825" height="355" loading="eager"></a><x-site-nav :items="$navItems" /><x-social-icons /><button class="menu-toggle" type="button" aria-label="Apri menu" aria-expanded="false"><span></span><span></span><span></span></button></div></div>
    <div class="wrap hero-inner"><div><div class="eyebrow">{{ $s?->hero_eyebrow ?? 'BOXE · DETTAGLIO · SPETTACOLO' }}</div><h1>{{ $s?->hero_line_1 ?? 'Una voce' }}<span>{{ $s?->hero_line_2 ?? 'oltre' }}</span><span>{{ $s?->hero_line_3 ?? 'il ring' }}</span></h1><div class="rule"></div><p>{{ $s?->hero_intro ?? 'Eventi, match, persone. Ogni grande spettacolo inizia con una grande voce.' }}</p><div class="hero-actions"><a class="btn" href="{{ $s?->hero_cta_url ?? ($pageLinks['chi-sono'] ?? '#bio') }}">{{ $s?->hero_cta_text ?? 'Scopri chi sono' }}</a><a class="play-link" href="{{ $s?->hero_video_url ?? '#video' }}"><span class="play">▶</span>{{ $s?->hero_video_text ?? 'Guarda il video' }}</a></div></div></div>
