...
<section id="contatti" class="cta"><div class="cta-bg"><img src="{{ $images['cta'] }}" alt="Corde del ring illuminate" loading="lazy" decoding="async"></div><div class="wrap cta-inner"><h2>{!! nl2br(e(str_replace(' THE ', "\nTHE ", $ctaTitle))) !!}</h2><p><strong>Contattami</strong>{{ $s?->cta_text ?? 'Per eventi, collaborazioni e informazioni' }}</p><a class="btn" href="{{ $s?->cta_button_url ?? ($pageLinks['contatti'] ?? 'mailto:info@ringannouncer.it') }}">{{ $s?->cta_button_text ?? 'Scrivimi' }}</a></div></section>
